  ...
  return new Promise((res, rej) => {
    ...
    _.forEach(buyData, (bv, bk) => {
        ...
        _.forEach(soldData, (sv, sk) => {
          ...
          switch (true) {
            ...
            case buyData[bk]["LOT ACREAGE"] > 3:
              ...
              break;
            case buyData[bk]["LOT ACREAGE"] > 0.5:
              minAcre = buyData[bk]["LOT ACREAGE"] - 0.5;
              maxAcre = buyData[bk]["LOT ACREAGE"] + 0.5;
              break;

            default:
              minAcre = buyData[bk]["LOT ACREAGE"] - 0.25;
              maxAcre = buyData[bk]["LOT ACREAGE"] + 0.25;
              break;
          }
          ...
          if (
            ...
            soldDistance < v + 1 &&
            soldDistance > v &&
            minAcre < soldProp["LOT ACREAGE"] &&
            maxAcre > soldProp["LOT ACREAGE"]
          ) {
      ...
      if (sortedDistanceSoldArr[0] && sortedDistanceSoldArr[1] && sortedDistanceSoldArr[2]) {
        totalPPADistance =
          sortedDistanceSoldArr[0]["PRICE PER ACRE"] +
          sortedDistanceSoldArr[1]["PRICE PER ACRE"] +
          sortedDistanceSoldArr[2]["PRICE PER ACRE"];
      ...
      if (sortedPPASoldArr[1] && sortedPPASoldArr[2] && sortedPPASoldArr[3]) {
        totalPPAMiddle =
          sortedPPASoldArr[1]["PRICE PER ACRE"] +
          sortedPPASoldArr[2]["PRICE PER ACRE"] +
          sortedPPASoldArr[3]["PRICE PER ACRE"];
      ...
      buyData[bk]["offer1"] = Math.floor((buyData[bk]["estValue"] * 0.5) / 100) * 100;
      ...
      buyData[bk]["marketValueFlag"] =
        buyData[bk]["statusColor"] !== "red" && buyData[bk]["MARKET TOTAL VALUE"] / buyData[bk]["estValue"] > 2
          ? true
          : false;
...
async function compareComps() {
  try {
    let buyD = await csvToJson()
      .fromFile("./csv/" + county + "/buy.csv")
      .then((data) => formatBuyData(data))
      .then((data) => createJSONFile(data, "buy"));
    soldD = await csvToJson()
      .fromFile("./csv/" + county + "/sold.csv")
      .then((data) => formatSoldData(data))
      .then((data) => createJSONFile(data, "sold"));
